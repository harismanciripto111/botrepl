...
def modechatgpt(apikey, txt, whoareyou):
    openai_api_key = apikey
    openai.api_key = openai_api_key
    messages=[
        {"role": "system", "content": whoareyou},
    ]
    item =  {"role": "user", "content": txt}
...
def main():
    ...
    openai_apikey = conf['OPENAI_KEY']
    youarea = conf['YOU_ARE']
            ...
            try:
                ...
                for chan in conf['CHANNEL_ID']:
                    ...
                    if mode == "quote":
                        ...
                        res = Bot.getMessage(chan, "1")
                        getlast = list(reversed(res))[0]
                        getMsg = modechatgpt(openai_apikey, getlast['content'], youarea)

                        if conf['REPLY']:
                            send = Bot.replyMessage(chan, getlast['id'], getMsg)
                            print("[{}][{}][CHATGPT] {}".format(me, chan, getMsg))
                        else:
